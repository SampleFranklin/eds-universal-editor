...
export default function decorate(block) {
  const [titleEl, subtitleEl, tabname1El, tabname2El, tabname3El, labelsEl] = block.children;

  // Debug: Log the elements to ensure they are correctly selected
  console.log('Title Element:', titleEl);
  console.log('Subtitle Element:', subtitleEl);

  const title = titleEl?.querySelector(':is(h1,h2,h3,h4,h5,h6)')?.textContent?.trim() || '';
  ...
  const [dealerNameEl, emailIdEl, scheduledDateEl, scheduledTimeEl, contactEl] = labelsEl.children;
  ...
  // Debug: Log the extracted text to ensure correctness
